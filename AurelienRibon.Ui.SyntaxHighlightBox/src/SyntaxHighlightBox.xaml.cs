	...
	public partial class SyntaxHighlightBox : TextBox
    {
		...
		private void DrawBlocks()
        {
			...
			for (int i = 0; i < blocks.Count; i++)
            {
				...
				double top = blocks[i].Position.Y - VerticalOffset;
				//double bottom = top + blockHeight;
				if (top < ActualHeight && top + blockHeight > 0)
                {
					try
                    {
						dc.DrawText(blocks[i].FormattedText, new Point(2 - HorizontalOffset, blocks[i].Position.Y - VerticalOffset));
						if (IsLineNumbersMarginVisible)
                        {
							lineNumbersCanvas.Width = GetFormattedTextWidth(string.Format("{0:0000}", totalLineCount)) + 5;
							dc2.DrawText(blocks[i].LineNumbers, new Point(lineNumbersCanvas.ActualWidth, 1 + blocks[i].Position.Y - VerticalOffset));
						}
					}
                    catch
                    {
		...
		private void FormatBlock(InnerTextBlock currentBlock, InnerTextBlock previousBlock)
        {
            ...
            var format = currentBlock.FormattedText;
            if (CurrentHighlighter != null)
            {
				ThreadPool.QueueUserWorkItem(p =>
                {
					int previousCode = previousBlock != null ? previousBlock.Code : -1;
					currentBlock.Code = CurrentHighlighter.Highlight(ref format, previousCode);
                    currentBlock.FormattedText = format;
                });
			}
		}
        ...
        public static readonly DependencyProperty TextForegroundProperty =
            DependencyProperty.Register("TextForeground", typeof(Brush), typeof(SyntaxHighlightBox), new PropertyMetadata(new SolidColorBrush(Colors.Black)));
